...
import net.dataninja.ee.textIndexer.tokenizer.XTFTokenizer;
import net.dataninja.ee.util.Attrib;
...
import net.dataninja.ee.util.XMLFormatter;
...
public abstract class TextServlet extends HttpServlet 
{
    ...
    throws ServletException, IOException 
  {
    ...
    try 
    {
      ...
      // In general it doesn't make sense to cache pages served by ee servlets.
   ...
   * to the given transformer. Also adds "ee.home" based on the servlet
   * root directory.
   */
   ...
   * "net.dataninja.ee.QueryProcessorClass" to see if there is a user-
  ...
  public QueryProcessor createQueryProcessor() 
  {
    ...
    final String propName = "net.dataninja.ee.QueryProcessorClass";
    ...
    try 
    {
      ...
      while (true) {
        ...
        assert ftok != null && ftok.termText().equals(tok.termText()) :
               "Internal ee bug: FastTokenizer isn't functioning the same as XTFTokenizer. " +
               "Please report this problem to the ee-user group, along with your query.";
  ...
  */
                              ...
                              Exception exc) 
  {
    ...
    try 
    {
      ...
      if (raw == null)
        raw = (String) req.getAttribute("net.dataninja.ee.servlet.raw");
